...
import ca.ampautomation.ampata.entity.SysPat;
...
public class FinAcctBrowse2 extends MasterDetailScreen<SysNode> {
    ...
    @Autowired
    protected PropertyFilter<SysNode> filterConfig1A_finTaxLne1_Id;
    ...
    @Autowired
    protected EntityComboBox<SysNode> tmplt_GenAgent1_IdField;
    ...
    @Autowired
    private TreeTable<SysNode> table;
    ...
    @Autowired
    private InstanceContainer<SysNode> finAcctDc;
    ...
    private CollectionContainer<SysNode> finAcct1sDc;

    private CollectionLoader<SysNode> finAcct1sDl;
    ...
    @Autowired
    private EntityComboBox<SysNode> genAgent1_IdField;
    ...
    @Autowired
    private EntityComboBox<SysNode> parent1_IdField;
 ...
 */
    ...
    public void onInit(InitEvent event) {
        ...
        //filter
        EntityComboBox<SysNode> propFilterCmpnt_finCurcy1_Id = (EntityComboBox<SysNode>) filterConfig1A_finCurcy1_Id.getValueComponent();
        ...
        finAcct1sDc = dataComponents.createCollectionContainer(SysNode.class);
        finAcct1sDl = dataComponents.createCollectionLoader();
        finAcct1sDl.setQuery("select e from ampata_SysNode e where e.className = 'FinAcct' order by e.id2");
        FetchPlan finAcct1sFp = fetchPlans.builder(SysNode.class)
                .addFetchPlan(FetchPlan.INSTANCE_NAME)
                .build();
        ...
        //filter
        EntityComboBox<SysNode> propFilterCmpnt_finTaxLne1_Id = (EntityComboBox<SysNode>) filterConfig1A_finTaxLne1_Id.getValueComponent();
    ...
    */
    ...
    public void onFinAcctsDcItemChange(InstanceContainer.ItemChangeEvent<SysNode> event) {
        String logPrfx = "onFinAcctsDcItemChange";
        logger.trace(logPrfx + " --> ");

        SysNode thisFinAcct = event.getItem();
        if (thisFinAcct == null) {
            logger.debug(logPrfx + " --- thisFinAcct is null, likely because no record is selected.");
            logger.trace(logPrfx + " <-- ");
            return;
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisFinAccts = table.getSelected().stream().toList();
    ...
    public void onSetBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> chngFinAccts = new ArrayList<>();
        List<SysNode> finalChngFinAccts = chngFinAccts;
    ...
    public void onRebuildSortIdxBtnClick(Button.ClickEvent event) {
        ...
        SysNode firstFinAcct = thisFinAccts.get(0);

        thisFinAccts = new ArrayList<>(getFinAcctsByParent1(firstFinAcct.getParent1_Id()));
        thisFinAccts.sort(Comparator.comparing(SysNode::getSortIdx,Comparator.nullsFirst(Comparator.naturalOrder())));
    ...
    public void onMoveFrstSortIdxBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisFinAccts = new ArrayList<>(table.getSelected().stream().toList());
        if (thisFinAccts == null || thisFinAccts.isEmpty()) {
            logger.debug(logPrfx + " --- thisFinAcct is null, likely because no records are selected.");
            notifications.create().withCaption("No records selected. Please select one or more record.").show();
            logger.trace(logPrfx + " <-- ");
        ...
        thisFinAccts.forEach(thisFinAcct -> {
            if (thisFinAcct != null) {
                ...
                if (sortIdx_ != null
                    ...
                    List<SysNode> nextFinAccts =  getFinAcctsBtwnSortIdx(-1
        ...
        thisFinAccts.forEach(thisFinAcct -> {
            ...
            if (thisFinAcct != null) {
                ...
                if (sortIdx_ != null
                    ...
                    SysNode prevFinAcct = getFinAcctBySortIdx(sortIdx_-1,thisFinAcct.getParent1_Id());
                    if(prevFinAcct != null){
        ...
        thisFinAccts.forEach(thisFinAcct -> {
            if (thisFinAcct != null) {
                ...
                if (sortIdxMax != null
                    ...
                    SysNode nextFinAcct = getFinAcctBySortIdx(sortIdx_+1,thisFinAcct.getParent1_Id());
                    if(nextFinAcct != null){
                        nextFinAcct = dataContext.merge(nextFinAcct);
                        Boolean nextFinAcctIsChanged = false;
    ...
    public void onMoveLastSortIdxBtnClick(Button.ClickEvent event) {
        ...
        //order descending
        thisFinAccts.sort(Comparator.comparing(SysNode::getSortIdx,Comparator.nullsFirst(Comparator.naturalOrder())).reversed());
        ...
        thisFinAccts.forEach(thisFinAcct -> {
            if (thisFinAcct != null) {
                ...
                if (sortIdx_ != null 
                    ...
                    List<SysNode> nextFinAccts =  getFinAcctsBtwnSortIdx(sortIdx_
    ...
    private void updateFinAcctHelper(List<SysNode> chngFinAccts) {
        ...
        if(chngFinAccts != null && !chngFinAccts.isEmpty()) {
            ...
            chngFinAccts.forEach(thisFinAcct -> {
                //SysNode thisTrackedFinAcct = dataContext.merge(thisFinAcct);
                if (thisFinAcct != null) {
                    thisFinAcct = dataContext.merge(thisFinAcct);
                    Boolean thisFinAcctIsChanged = false;
    ...
    private Integer getSortIdxMax(SysNode thisFinAcct) {
    ...
    public void onUpdateInstIdPartsBtnClick(Button.ClickEvent event) {
        ...
        SysNode thisFinAcct = finAcctDc.getItemOrNull();
        if (thisFinAcct == null) {
            logger.debug(logPrfx + " --- thisFinAcct is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    private Boolean updateCalcVals(@NotNull SysNode thisFinAcct, Integer option) {
    ...
    private Boolean updateIdParts(@NotNull SysNode thisFinAcct) {
    ...
    private Boolean updateId2(@NotNull SysNode thisFinAcct) {
    ...
    private Boolean updateId2Calc(@NotNull SysNode thisFinAcct) {
        // Assume thisFinAcct is not null
    ...
    private Boolean updateId2Cmp(@NotNull SysNode thisFinAcct) {
        // Assume thisFinAcct is not null
    ...
    private Boolean updateId2Dup(@NotNull SysNode thisFinAcct) {
        // Assume thisFinAcct is not null
        ...
        if (thisFinAcct.getId2() != null) {
            String id2Qry = "select count(e) from ampata_SysNode e where e.className = 'FinAcct' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try {
    ...
    private Boolean updatetName1(@NotNull SysNode thisFinAcct) {
        // Assume thisFinAcct is not null
    ...
    private Boolean updatetAgent1(@NotNull SysNode thisFinAcct) {
        // Assume thisFinAcct is not null
    ...
    private SysNode getFinAcctBySortIdx(@NotNull Integer sortIdx, SysNode parent1_Id) {
        String logPrfx = "getFinAcctBySortIdx";
        logger.trace(logPrfx + " --> ");

        String qry = "select e from ampata_SysNode e where e.className = 'FinAcct'"
        ...
        SysNode finAcct = null;
        try {
            finAcct = dataManager.load(SysNode.class)
                    .query(qry)
    ...
    private List<SysNode> getFinAcctsBtwnSortIdx(@NotNull Integer sortIdxA , @NotNull Integer sortIdxB, SysNode parent1_Id) {
        ...
        List<SysNode> finAccts = null;
        try {
            finAccts = dataManager.load(SysNode.class)
                    .query(qry)
                    .parameter("parent1_Id", parent1_Id)
    ...
    private List<SysNode> getFinAcctsByParent1(SysNode parent1_Id) {
