...
public class FinBalSetBrowse2 extends MasterDetailScreen<SysNode> {
    ...
    @Autowired
    protected PropertyFilter<SysNode> propFilter_finDept1_Id;
    
    @Autowired
    protected PropertyFilter<SysNodeType> propFilter2Config1_type1_Id;
    ...
    @Autowired
    private Table<SysNode> tableFinAcct;
    ...
    @Autowired
    private CollectionContainer<SysNode> finBalSetsDc;

    @Autowired
    private CollectionLoader<SysNode> finBalSetsDl;

    @Autowired
    private InstanceContainer<SysNode> finBalSetDc;


    private CollectionContainer<SysNodeType> finBalSetTypesDc;

    private CollectionLoader<SysNodeType> finBalSetTypesDl;
    ...
    private CollectionContainer<SysNode> finBalSet1sDc;

    private CollectionLoader<SysNode> finBalSet1sDl;
    ...
    private CollectionContainer<SysNodeType> finAcctTypesDc;

    private CollectionLoader<SysNodeType> finAcctTypesDl;
    ...
    @Autowired
    private EntityComboBox<SysNode> finBalSet1_IdField;
    ...
    @Autowired
    private Table<SysNode> tableFinBal;

    @Autowired
    private CollectionContainer<SysNode> finBalsDc;

    @Autowired
    private CollectionLoader<SysNode> finBalsDl;
    ...
    public void onInit(InitEvent event) {
        ...
        finBalSetTypesDc = dataComponents.createCollectionContainer(SysNodeType.class);
        finBalSetTypesDl = dataComponents.createCollectionLoader();
        finBalSetTypesDl.setQuery("select e from ampata_SysNodeType e where e.className = 'FinBalSet' order by e.id2");
        FetchPlan finBalSetTypesFp = fetchPlans.builder(SysNodeType.class)
        ...
        //filter
        EntityComboBox<SysNode> propFilterCmpnt_finDept1_Id = (EntityComboBox<SysNode>) propFilter_finDept1_Id.getValueComponent();
        propFilterCmpnt_finDept1_Id.setOptionsContainer(finDeptsDc);


        finBalSet1sDc = dataComponents.createCollectionContainer(SysNode.class);
        finBalSet1sDl = dataComponents.createCollectionLoader();
        finBalSet1sDl.setQuery("select e from ampata_SysNode e where e.className = 'FinBalSet' order by e.id2");
        FetchPlan finBalSet1sFp = fetchPlans.builder(SysNode.class)
                .addFetchPlan(FetchPlan.INSTANCE_NAME)
                .build();
        ...
        finAcctTypesDc = dataComponents.createCollectionContainer(SysNodeType.class);
        finAcctTypesDl = dataComponents.createCollectionLoader();
        finAcctTypesDl.setQuery("select e from ampata_SysNodeType e where e.className = 'FinAcct' order by e.id2");
        FetchPlan finAcctTypesFp = fetchPlans.builder(SysNodeType.class)
                .addFetchPlan(FetchPlan.INSTANCE_NAME)
                .build();
        ...
        //filter
        EntityComboBox<SysNodeType> propFilterCmpnt2Config1_type1_Id = (EntityComboBox<SysNodeType>) propFilter2Config1_type1_Id.getValueComponent();
    ...
    public void onFinBalSetsDcCollectionChange(CollectionContainer.CollectionChangeEvent<SysNode> event) {
    ...
    @Subscribe(id = "finBalSetsDc", target = Target.DATA_CONTAINER)
    public void onFinBalSetsDcItemChange(InstanceContainer.ItemChangeEvent<SysNode> event) {
        String logPrfx = "onFinBalSetsDcItemChange";
        logger.trace(logPrfx + " --> ");

        SysNode thisFinBalSet = event.getItem();
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisFinBalSets = table.getSelected().stream().toList();
    ...
    public void onSetBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> chngFinBalSets = new ArrayList<>();
        List<SysNode> finalChngFinBalSets = chngFinBalSets;
    ...
    public void onRebuildSortIdxBtnClick(Button.ClickEvent event) {
        ...
        SysNode firstFinBalSet = thisFinBalSets.get(0);

        thisFinBalSets = new ArrayList<>(getFinBalSetsByParent1(firstFinBalSet.getParent1_Id()));
        thisFinBalSets.sort(Comparator.comparing(SysNode::getSortIdx,Comparator.nullsFirst(Comparator.naturalOrder())));
        ...
        thisFinBalSets.forEach(thisFinBalSet -> {
            if (thisFinBalSet != null) {
                SysNode thisTrackedFinBalSet = dataContext.merge(thisFinBalSet);
    ...
    public void onMoveFrstSortIdxBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisFinBalSets = new ArrayList<>(table.getSelected().stream().toList());
        if (thisFinBalSets == null || thisFinBalSets.isEmpty()) {
            logger.debug(logPrfx + " --- thisFinBalSet is null, likely because no records are selected.");
            notifications.create().withCaption("No records selected. Please select one or more record.").show();
            logger.trace(logPrfx + " <-- ");
        ...
        thisFinBalSets.forEach(thisFinBalSet -> {
            if (thisFinBalSet != null) {
                ...
                if (sortIdx_ != null
                    ...
                    List<SysNode> nextFinBalSets =  getFinBalSetsBtwnSortIdx(-1
        ...
        thisFinBalSets.forEach(thisFinBalSet -> {
            ...
            if (thisFinBalSet != null) {
                ...
                if (sortIdx_ != null
                    ...
                    SysNode prevFinBalSet = getFinBalSetBySortIdx(sortIdx_-1,thisFinBalSet.getParent1_Id());
        ...
        thisFinBalSets.forEach(thisFinBalSet -> {
            if (thisFinBalSet != null) {
                ...
                if (sortIdxMax != null
                    ...
                    SysNode nextFinBalSet = getFinBalSetBySortIdx(sortIdx_+1,thisFinBalSet.getParent1_Id());
                    if(nextFinBalSet != null){
                        nextFinBalSet = dataContext.merge(nextFinBalSet);
                        Boolean nextFinBalSetIsChanged = false;
    ...
    public void onMoveLastSortIdxBtnClick(Button.ClickEvent event) {
        ...
        //order descending
        thisFinBalSets.sort(Comparator.comparing(SysNode::getSortIdx,Comparator.nullsFirst(Comparator.naturalOrder())).reversed());
        ...
        thisFinBalSets.forEach(thisFinBalSet -> {
            if (thisFinBalSet != null) {
                ...
                if (sortIdx_ != null
                    ...
                    List<SysNode> nextFinBalSets =  getFinBalSetsBtwnSortIdx(sortIdx_
    ...
    private void updateFinBalSetHelper(List<SysNode> chngFinBalSets) {
    ...
    private Integer getSortIdxMax(SysNode thisFinBalSet) {
        String logPrfx = "getSortIdxMax";
        logger.trace(logPrfx + " --> ");

        Integer sortIdxMax = null;
        String sortIdxMaxQry = "select max(e.sortIdx) from ampata_SysNode e"
    ...
    public void onFinBalSetDcItemChange(InstanceContainer.ItemChangeEvent<SysNode> event) {
        String logPrfx = "onFinBalSetDcItemChange";
        logger.trace(logPrfx + " --> ");

        SysNode thisFinBalSet = event.getSource().getItemOrNull();
        if (thisFinBalSet == null) {
    ...
    public void onUpdateDesc1FieldBtn(Button.ClickEvent event) {
        ...
        SysNode thisFinBalSet = finBalSetDc.getItemOrNull();
        if (thisFinBalSet == null) {
            logger.debug(logPrfx + " --- thisFinBalSet is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    public void onAddSelTableFinAcctBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisFinAccts = new ArrayList<>(tableFinAcct.getSelected().stream().toList());
    ...
    public void onAddAllTableFinAcctBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisFinAccts = tableFinAcct.getItems().getItems().stream().toList();
        if (thisFinAccts == null || thisFinAccts.isEmpty()) {
    ...
    public Integer addFinAccts(@NotNull SysNode thisFinBalSet, List<SysNode> thisFinAccts, int iOptions){
            ...
            if (thisFinAcct != null) {
                ...
                if (Objects.equals(finAcctType1_Id2, "/") || Objects.equals(finAcctType1_Id2, "/Grp")) {
                    ...
                    try{
                        ...
                        finBals1_QryCnt = "select count(e) from ampata_SysNode e"
                    ...
                    if (finBals1_IdCntCalc > 0){
                        ...
                        SysNode finBal = dataManager.create(SysNode.class);
    ...
    public Boolean updateCalcVals(SysNode thisFinBalSet){
    ...
    private Boolean updateFinBalSetCalcVals(@NotNull SysNode thisFinBalSet) {
    ...
    private Boolean updateIdParts(@NotNull SysNode thisFinBalSet) {
    ...
    private Boolean updateId2(@NotNull SysNode thisFinBalSet) {
    ...
    private Boolean updateId2Calc(SysNode thisFinBalSet){
        // Assume thisFinBalSet is not null
    ...
    private Boolean updateId2Cmp(@NotNull SysNode thisFinBalSet) {
        // Assume thisFinBalSet is not null
    ...
    private Boolean updateId2Dup(@NotNull SysNode thisFinBalSet) {
        // Assume thisFinBalSet is not null
        ...
        if (thisFinBalSet.getId2() != null){
            String id2Qry = "select count(e) from ampata_SysNode e where e.className = 'FinBalSet' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try{
    ...
    private Boolean updateDesc1(@NotNull SysNode thisFinBalSet){
        // Assume thisFinBalSet is not null
    ...
    private Boolean updateIdDt(@NotNull SysNode thisFinBalSet) {
        // Assume thisFinBalSet is not null
    ...
    private Boolean updateEnd1(@NotNull SysNode thisFinBalSet) {
        // Assume thisFinBalSet is not null
    ...
    private SysNode findFinBalSetById2(@NotNull String FinBalSet_Id2) {
        ...
        String qry = "select e from ampata_SysNode e where e.className = 'FinBalSet' and e.id2 = :id2";
        logger.debug(logPrfx + " --- qry: " + qry);
        logger.debug(logPrfx + " --- qry:id2: " + FinBalSet_Id2);

        SysNode FinBalSet1_Id = null;
        try {
            FinBalSet1_Id = dataManager.load(SysNode.class)
                    .query(qry)
    ...
    private SysNode getFinBalSetBySortIdx(@NotNull Integer sortIdx, SysNode parent1_Id) {
        String logPrfx = "getFinBalSetBySortIdx";
        logger.trace(logPrfx + " --> ");

        String qry = "select e from ampata_SysNode e where e.className = 'FinBalSet'"
        ...
        SysNode finBalSet = null;
        try {
            finBalSet = dataManager.load(SysNode.class)
                    .query(qry)
    ...
    private List<SysNode> getFinBalSetsBtwnSortIdx(@NotNull Integer sortIdxA , @NotNull Integer sortIdxB, SysNode parent1_Id) {
        ...
        List<SysNode> finBalSets = null;
        try {
            finBalSets = dataManager.load(SysNode.class)
                    .query(qry)
                    .parameter("parent1_Id", parent1_Id)
    ...
    private List<SysNode> getFinBalSetsByParent1(SysNode parent1_Id) {
    ...
    private SysNodeType getFinBalTypeById2(@NotNull String id2) {
        String logPrfx = "getFinBalTypeById2";
        logger.trace(logPrfx + " --> ");

        String qry = "select e from ampata_SysNodeType e where e.className = 'FinBal'"
                + " and e.id2 = :id2"
                ;
        logger.debug(logPrfx + " --- qry: " + qry);
        logger.debug(logPrfx + " --- qry:id2: " + id2);

        SysNodeType finBalType = null;
        try {
            finBalType = dataManager.load(SysNodeType.class)
                    .query(qry)
                    .parameter("id2", id2)
                    .one();
            logger.debug(logPrfx + " --- query qry returned ONE result");
    ...
    private SysNode getFinBalByFinBalSet1_FinAcct1(@NotNull SysNode finBalSet1, @NotNull SysNode finAcct1) {
        String logPrfx = "getFinBalByFinBalSet1_FinAcct1";
        logger.trace(logPrfx + " --> ");

        String qry = "select e from ampata_SysNode e where e.className = 'FinBal'"
        ...
        SysNode finBal = null;
        try {
            finBal = dataManager.load(SysNode.class)
                    .query(qry)
    ...
    public void loadTableFinBal(@NotNull SysNode thisFinBalSet) {
        String logPrfx = "loadTableFinBal";
        logger.trace(logPrfx + " --> ");

        finBalsDc = dataComponents.createCollectionContainer(SysNode.class);
        finBalsDl = dataComponents.createCollectionLoader();

        try{
            String finTxactItms1_Qry;
            finTxactItms1_Qry = "select e from ampata_SysNode e"
                    + " where e.className = 'FinBal'"
                    + " and e.finBalSet1_Id = :finBalSet1_Id"
                    + "";
            ...
            FetchPlan finBalsFp = fetchPlans.builder(SysNode.class)
