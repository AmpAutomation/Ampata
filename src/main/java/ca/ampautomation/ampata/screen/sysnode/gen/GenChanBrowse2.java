...
public class GenChanBrowse2 extends MasterDetailScreen<SysNode> {
    ...
    @Autowired
    private InstanceContainer<SysNode> genChanDc;


    private CollectionContainer<SysNodeType> genChanTypesDc;

    private CollectionLoader<SysNodeType> genChanTypesDl;
    ...
    public void onInit(InitEvent event) {
        ...
        genChanTypesDc = dataComponents.createCollectionContainer(SysNodeType.class);
        genChanTypesDl = dataComponents.createCollectionLoader();
        genChanTypesDl.setQuery("select e from ampata_SysNodeType e where e.className = 'GenChan' order by e.id2");
        FetchPlan genChanTypesFp = fetchPlans.builder(SysNodeType.class)
    ...
    public void onGenChansDcCollectionChange(CollectionContainer.CollectionChangeEvent<SysNode> event) {
    ...
    @Subscribe(id = "genChansDc", target = Target.DATA_CONTAINER)
    public void onGenChansDcItemChange(InstanceContainer.ItemChangeEvent<SysNode> event) {
        String logPrfx = "onGenChansDcItemChange";
        logger.trace(logPrfx + " --> ");

        SysNode thisGenChan = event.getItem();
        if (thisGenChan == null) {
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<SysNode> thisGenChans = table.getSelected().stream().toList();
    ...
    public void onSetBtnClick(Button.ClickEvent event) {
        ...
        thisGenChans.forEach(thisGenChan -> {
            SysNode thisTrackedGenChan = dataContext.merge(thisGenChan);
    ...
    public void onGenChanDcItemChange(InstanceContainer.ItemChangeEvent<SysNode> event) {
        String logPrfx = "onGenChanDcItemChange";
        logger.trace(logPrfx + " --> ");

        SysNode thisGenChan = event.getSource().getItemOrNull();
        if (thisGenChan == null) {
    ...
    public void onUpdateDesc1FieldBtn(Button.ClickEvent event) {
        ...
        SysNode thisGenChan = genChanDc.getItemOrNull();
        if (thisGenChan == null) {
            logger.debug(logPrfx + " --- thisGenChan is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    public Boolean updateCalcVals(SysNode thisGenChan){
    ...
    private Boolean updateGenChanCalcVals(@NotNull SysNode thisGenChan) {
    ...
    private Boolean updateIdParts(@NotNull SysNode thisGenChan) {
    ...
    private Boolean updateId2(@NotNull SysNode thisGenChan) {
    ...
    private Boolean updateId2Calc(SysNode thisGenChan){
        // Assume thisGenChan is not null
    ...
    private Boolean updateId2Cmp(@NotNull SysNode thisGenChan) {
        // Assume thisGenChan is not null
    ...
    private Boolean updateId2Dup(@NotNull SysNode thisGenChan) {
        // Assume thisGenChan is not null
        ...
        if (thisGenChan.getId2() != null){
            String id2Qry = "select count(e) from ampata_SysNode e where e.className = 'GenChan' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try{
    ...
    private Boolean updateDesc1(@NotNull SysNode thisGenChan){
        // Assume thisGenChan is not null
    ...
    private SysNode findGenChanById2(@NotNull String GenChan_Id2) {
        ...
        String qry = "select e from ampata_SysNode e where e.className = 'GenChan' and e.id2 = :id2";
        logger.debug(logPrfx + " --- qry: " + qry);
        logger.debug(logPrfx + " --- qry:id2: " + GenChan_Id2);

        SysNode GenChan1_Id = null;
        try {
            GenChan1_Id = dataManager.load(SysNode.class)
                    .query(qry)
