...
public class FinCurcyBrowse2 extends MasterDetailScreen<GenNode> {
    ...
    @Autowired
    private InstanceContainer<GenNode> finCurcyDc;



    private CollectionContainer<GenNodeType> finCurcyTypesDc;

    private CollectionLoader<GenNodeType> finCurcyTypesDl;
...
*/
    ...
    public void onInit(InitEvent event) {
        ...
        finCurcyTypesDc = dataComponents.createCollectionContainer(GenNodeType.class);
        finCurcyTypesDl = dataComponents.createCollectionLoader();
        finCurcyTypesDl.setQuery("select e from ampata_GenNodeType e where e.className = 'FinCurcy' order by e.id2");
        FetchPlan finCurcyTypesFp = fetchPlans.builder(GenNodeType.class)
    ...
    */
    ...
    public void onFinCurcysDcItemChange(InstanceContainer.ItemChangeEvent<GenNode> event) {
        String logPrfx = "onFinCurcysDcItemChange";
        logger.trace(logPrfx + " --> ");

        GenNode thisFinCurcy = event.getItem();
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<GenNode> thisFinCurcys = table.getSelected().stream().toList();
    ...
    public void onSetBtnClick(Button.ClickEvent event) {
        ...
        List<GenNode> chngFinCurcys = new ArrayList<>();
        List<GenNode> finalChngFinCurcys = chngFinCurcys;
    ...
    private void updateFinCurcyHelper(List<GenNode> chngFinCurcys) {
        ...
        if(chngFinCurcys != null && !chngFinCurcys.isEmpty()) {
            ...
            chngFinCurcys.forEach(thisFinCurcy -> {
                //GenNode thisTrackedFinCurcy = dataContext.merge(thisFinCurcy);
                if (thisFinCurcy != null) {
                    thisFinCurcy = dataContext.merge(thisFinCurcy);
                    Boolean thisFinCurcyIsChanged = false;
    ...
    public void onUpdateInstItemValsBtnClick(Button.ClickEvent event) {
        ...
        GenNode thisFinCurcy = finCurcyDc.getItemOrNull();
        if (thisFinCurcy == null) {
            logger.debug(logPrfx + " --- thisFinCurcy is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    private Boolean updateCalcVals(@NotNull GenNode thisFinCurcy) {
    ...
    private Boolean updateId2(@NotNull GenNode thisFinCurcy) {
    ...
    private Boolean updateId2Calc(@NotNull GenNode thisFinCurcy) {
        // Assume thisFinCurcy is not null
    ...
    private Boolean updateId2Cmp(@NotNull GenNode thisFinCurcy) {
        // Assume thisFinCurcy is not null
    ...
    private Boolean updateId2Dup(@NotNull GenNode thisFinCurcy) {
        // Assume thisFinCurcy is not null
        ...
        if (thisFinCurcy.getId2() != null) {
            String id2Qry = "select count(e) from ampata_GenNode e where e.className = 'FinCurcy' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try {
