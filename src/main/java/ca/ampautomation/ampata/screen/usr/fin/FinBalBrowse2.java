...
public class FinBalBrowse2 extends MasterDetailScreen<UsrNode> {
    ...
    @Autowired
    protected PropertyFilter<UsrNode> propFilter_finBalSet1_Id;

    @Autowired
    protected PropertyFilter<UsrNode> propFilter_finAcct1_Id;

    @Autowired
    protected PropertyFilter<UsrNode> propFilter_finDept1_Id;

    @Autowired
    protected PropertyFilter<SysNode> propFilter_sysFinCurcy1_Id;
    ...
    @Autowired
    private CollectionContainer<UsrNode> finBalsDc;

    @Autowired
    private CollectionLoader<UsrNode> finBalsDl;

    @Autowired
    private InstanceContainer<UsrNode> finBalDc;


    private CollectionContainer<UsrNodeType> finBalTypesDc;

    private CollectionLoader<UsrNodeType> finBalTypesDl;


    private CollectionContainer<UsrNode> finBalSetsDc;

    private CollectionLoader<UsrNode> finBalSetsDl;
    ...
    private CollectionContainer<UsrNode> finBal1sDc;

    private CollectionLoader<UsrNode> finBal1sDl;
    ...
    public void onInit(InitEvent event) {
        ...
        finBalTypesDc = dataComponents.createCollectionContainer(UsrNodeType.class);
        finBalTypesDl = dataComponents.createCollectionLoader();
        finBalTypesDl.setQuery("select e from ampata_UsrNodeType e where e.className = 'FinBal' order by e.id2");
        ...
        finBalSetsDc = dataComponents.createCollectionContainer(UsrNode.class);
        finBalSetsDl = dataComponents.createCollectionLoader();
        finBalSetsDl.setQuery("select e from ampata_UsrNode e where e.className = 'FinBalSet' order by e.id2");
        ...
        EntityComboBox<UsrNode> propFilterCmpnt_finBalSet1_Id = (EntityComboBox<UsrNode>) propFilter_finBalSet1_Id.getValueComponent();
        ...
        //filter
        EntityComboBox<UsrNode> propFilterCmpnt_finDept1_Id = (EntityComboBox<UsrNode>) propFilter_finDept1_Id.getValueComponent();
        ...
        //filter
        EntityComboBox<SysNode> propFilterCmpnt_sysFinCurcy1_Id = (EntityComboBox<SysNode>) propFilter_sysFinCurcy1_Id.getValueComponent();
        propFilterCmpnt_sysFinCurcy1_Id.setOptionsContainer(sysFinCurcysDc);


        finBal1sDc = dataComponents.createCollectionContainer(UsrNode.class);
        finBal1sDl = dataComponents.createCollectionLoader();
        finBal1sDl.setQuery("select e from ampata_UsrNode e where e.className = 'FinBal' order by e.id2");
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<UsrNode> thisFinBals = table.getSelected().stream().toList();
    ...
    public void onUpdateDesc1FieldBtn(Button.ClickEvent event) {
        ...
        UsrNode thisFinBal = finBalDc.getItemOrNull();
        if (thisFinBal == null) {
            logger.debug(logPrfx + " --- thisFinBal is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    private Boolean updateId2Dup(@NotNull UsrNode thisFinBal) {
        ...
        if (thisFinBal.getId2() != null){
            String id2Qry = "select count(e) from ampata_UsrNode e where e.className = 'FinBal' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try{
    ...
    public Boolean updateFinTxactItms1_FieldSet(@NotNull UsrNode thisFinBal) {
        ...
        try{
            ...
            if (finDept1 == null) {
                finTxactItms1_QryCnt = "select count(e) from ampata_UsrNode e"
        ...
        try{
            ...
            if (finDept1 == null) {
                finTxactItms1_QryDebt = "select sum(e.amtDebt) from ampata_UsrNode e"
                        + " where e.className = 'FinTxactItm'"
                        + " and e.finAcct1_Id = :finAcct1_Id"
                        + " and e.idDt.date1 between :begDt and :endDt"
                        + "";
        ...
        try {
            ...
            if (finDept1 == null) {
                finTxactItms1_QryCred = "select sum(e.amtCred) from ampata_UsrNode e"
                        + " where e.className = 'FinTxactItm'"
                        + " and e.finAcct1_Id = :finAcct1_Id"
                        + " and e.idDt.date1 between :begDt and :endDt"
                        + "";
    ...
    private UsrNode findFinBalById2(@NotNull String FinBal_Id2) {
        ...
        String qry = "select e from ampata_UsrNode e where e.className = 'FinBal' and e.id2 = :id2";
    ...
    public void loadTableFinTxactItm(@NotNull UsrNode thisFinBal) {
        ...
        try{
            ...
            if (finDept1 == null) {
                finTxactItms1_Qry = "select e from ampata_UsrNode e"
                        + " where e.className = 'FinTxactItm'"
                        + " and e.finAcct1_Id = :finAcct1_Id"
                        + " and e.idDt.date1 between :begDt and :endDt"
                        + "";
