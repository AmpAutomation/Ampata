...
@UiController("ampata_UsrGenChan.browse2")
@UiDescriptor("usr-gen-chan-browse2.xml")
@LookupComponent("table")
public class UsrGenChanBrowse2 extends MasterDetailScreen<UsrNode> {

    //Common
    Logger logger = LoggerFactory.getLogger(UsrGenChanBrowse2.class);
    ...
    //Main data loader, containers and table
    @Autowired
    private CollectionLoader<UsrNode> genChansDl;
    @Autowired
    private CollectionContainer<UsrNode> genChansDc;
    @Autowired
    private InstanceContainer<UsrNode> genChanDc;
    @Autowired
    private Table<UsrNode> table;


    //Other data loaders, containers and tables
    private CollectionContainer<UsrNodeType> genChanTypesDc;

    private CollectionLoader<UsrNodeType> genChanTypesDl;
    ...
    public void onInit(InitEvent event) {
        ...
        genChanTypesDc = dataComponents.createCollectionContainer(UsrNodeType.class);
        genChanTypesDl = dataComponents.createCollectionLoader();
        genChanTypesDl.setQuery("select e from ampata_UsrNodeType e where e.className = 'GenChan' order by e.id2");
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<UsrNode> thisGenChans = table.getSelected().stream().toList();
    ...
    public void onUpdateDesc1FieldBtn(Button.ClickEvent event) {
        ...
        UsrNode thisGenChan = genChanDc.getItemOrNull();
        if (thisGenChan == null) {
            logger.debug(logPrfx + " --- thisGenChan is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    private Boolean updateId2Dup(@NotNull UsrNode thisGenChan) {
        ...
        if (thisGenChan.getId2() != null){
            String id2Qry = "select count(e) from ampata_UsrNode e where e.className = 'GenChan' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try{
    ...
    private UsrNode findGenChanById2(@NotNull String GenChan_Id2) {
        ...
        String qry = "select e from ampata_UsrNode e where e.className = 'GenChan' and e.id2 = :id2";
