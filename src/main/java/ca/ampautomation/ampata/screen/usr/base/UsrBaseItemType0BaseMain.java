...
import ca.ampautomation.ampata.entity.usr.base.UsrBaseItemType;
import io.jmix.core.*;
...
public  abstract class UsrBaseItemType0BaseMain<ItemTypeT extends UsrBaseItemType, ItemTypeQryMngrT extends UsrBaseQryMngr, TableT extends Table> extends MasterDetailScreen<ItemTypeT> {
    ...
    public UsrBaseItemType0BaseMain() {
    ...
    @Autowired
    protected CollectionLoader<UsrBaseItemType> colLoadrMain;
    @Autowired
    protected CollectionContainer<UsrBaseItemType> colCntnrMain;
    @Autowired
    protected InstanceContainer<UsrBaseItemType> instCntnrMain;
    @Autowired
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<UsrBaseItemType> thisItemTypes = tableMain.getSelected().stream().toList();
        ...
        thisItemTypes.forEach(orig -> {
            UsrBaseItemType copy = metadataTools.copy(orig);
            ...
            UsrBaseItemType savedCopy = dataManager.save(copy);
    ...
    protected void updateHelper() {
        ...
        if(dataContext.hasChanges()) {
            ...
            thisItemTypes.forEach(thisItemType -> {
                //UsrBaseItemType thisTrackedItem = dataContext.merge(thisItemType);
                if (thisItemType != null) {
                    thisItemType = dataContext.merge(thisItemType);
    ...
    public void onUpdateColItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        List<UsrBaseItemType> thisItems = tableMain.getSelected().stream().toList();
    ...
    public void onInstCntnrMainItemChange(InstanceContainer.ItemChangeEvent<UsrBaseItemType> event) {
        String logPrfx = "onInstCntnrMainItemChange";
        logger.trace(logPrfx + " --> ");

        UsrBaseItemType thisItemType = event.getSource().getItemOrNull();
        if (thisItemType == null) {
    ...
    public void onUpdateInstItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        UsrBaseItemType thisItemType = instCntnrMain.getItemOrNull();
        if (thisItemType == null) {
            logger.debug(logPrfx + " --- thisItemType is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
