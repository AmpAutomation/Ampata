...
public abstract class UsrBaseItem0BaseMain<ItemT extends UsrBaseItem, ItemTypeT extends UsrBaseItemType, ItemQryMngrT extends UsrBaseQryMngr, TableT extends Table> extends MasterDetailScreen<ItemT> {
    ...
    protected void updateHelper2(List<ItemT> chngItems) {
        ...
        if(chngItems != null && !chngItems.isEmpty()) {
            ...
            chngItems.forEach(thisItem -> {
                //UsrBaseItem thisTrackedFinAcct = dataContext.merge(thisItem);
                if (thisItem != null) {
                    thisItem = dataContext.merge(thisItem);
                    Boolean thisItemIsChanged = false;
    ...
    protected void updateHelper(List<UsrBaseItem> chngItems) {
        String logPrfx = "updateHelper";
        logger.trace(logPrfx + " --> ");
    ...
    public void onUpdateName1FieldBtnClick(Button.ClickEvent event) {
        ...
        UsrBaseItem thisItem = instCntnrMain.getItemOrNull();
        if (thisItem == null) {
            logger.debug(logPrfx + " --- thisItem is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
