...
public abstract class UsrEdgeBase0BaseMain<EdgeT extends UsrEdgeBase, EdgeTypeT extends UsrEdgeBaseType, EdgeQryMngrT extends UsrComnBaseQryMngr, TableT extends Table> extends MasterDetailScreen<EdgeT> {
    ...
    protected void updateHelper2(List<EdgeT> chngEdges) {
        ...
        if(chngEdges != null && !chngEdges.isEmpty()) {
            ...
            chngEdges.forEach(thisEdge -> {
                //UsrEdgeBase thisTrackedFinAcct = dataContext.merge(thisEdge);
                if (thisEdge != null) {
                    thisEdge = dataContext.merge(thisEdge);
                    Boolean thisEdgeIsChanged = false;
    ...
    protected void updateHelper(List<UsrEdgeBase> chngEdges) {
        String logPrfx = "updateHelper";
        logger.trace(logPrfx + " --> ");
