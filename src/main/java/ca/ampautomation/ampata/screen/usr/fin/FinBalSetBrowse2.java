...
public class FinBalSetBrowse2 extends MasterDetailScreen<UsrNode> {
    ...
    @Autowired
    protected PropertyFilter<UsrNodeType> propFilter2Config1_type1_Id;
    ...
    @Autowired
    private InstanceContainer<UsrNode> finBalSetDc;
    ...
    public void onInit(InitEvent event) {
        ...
        finBalSetTypesDl.setQuery("select e from ampata_UsrNodeType e where e.className = 'FinBalSet' order by e.id2");
        ...
        finBalSet1sDl.setQuery("select e from ampata_UsrNode e where e.className = 'FinBalSet' order by e.id2");
        ...
        //filter
        EntityComboBox<UsrNodeType> propFilterCmpnt2Config1_type1_Id = (EntityComboBox<UsrNodeType>) propFilter2Config1_type1_Id.getValueComponent();
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<UsrNode> thisFinBalSets = table.getSelected().stream().toList();
    ...
    public void onMoveFrstSortIdxBtnClick(Button.ClickEvent event) {
        ...
        List<UsrNode> thisFinBalSets = new ArrayList<>(table.getSelected().stream().toList());
        if (thisFinBalSets == null || thisFinBalSets.isEmpty()) {
            logger.debug(logPrfx + " --- thisFinBalSet is null, likely because no records are selected.");
            notifications.create().withCaption("No records selected. Please select one or more record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    public void onUpdateDesc1FieldBtn(Button.ClickEvent event) {
        ...
        UsrNode thisFinBalSet = finBalSetDc.getItemOrNull();
        if (thisFinBalSet == null) {
            logger.debug(logPrfx + " --- thisFinBalSet is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
            ...
            if (thisFinAcct != null) {
                ...
                if (Objects.equals(finAcctType1_Id2, "/") || Objects.equals(finAcctType1_Id2, "/Grp")) {
                    ...
                    try{
                        ...
                        finBals1_QryCnt = "select count(e) from ampata_UsrNode e"
    ...
    private Boolean updateId2Dup(@NotNull UsrNode thisFinBalSet) {
        ...
        if (thisFinBalSet.getId2() != null){
            String id2Qry = "select count(e) from ampata_UsrNode e where e.className = 'FinBalSet' and e.id2 = :id2 and e.id <> :id";
            Integer id2Dup;
            try{
    ...
    private UsrNode findFinBalSetById2(@NotNull String FinBalSet_Id2) {
        ...
        String qry = "select e from ampata_UsrNode e where e.className = 'FinBalSet' and e.id2 = :id2";
    ...
    private UsrNode getFinBalSetBySortIdx(@NotNull Integer sortIdx, UsrNode parent1_Id) {
        ...
        String qry = "select e from ampata_UsrNode e where e.className = 'FinBalSet'"
    ...
    private UsrNodeType getFinBalTypeById2(@NotNull String id2) {
        ...
        String qry = "select e from ampata_UsrNodeType e where e.className = 'FinBal'"
                + " and e.id2 = :id2"
                ;
        logger.debug(logPrfx + " --- qry: " + qry);
    ...
    private UsrNode getFinBalByFinBalSet1_FinAcct1(@NotNull UsrNode finBalSet1, @NotNull UsrNode finAcct1) {
        ...
        String qry = "select e from ampata_UsrNode e where e.className = 'FinBal'"
    ...
    public void loadTableFinBal(@NotNull UsrNode thisFinBalSet) {
        ...
        finBalsDc = dataComponents.createCollectionContainer(UsrNode.class);
