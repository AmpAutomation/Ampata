...
public abstract class UsrNodeBase0Type0BaseMain<NodeTypeT extends UsrNodeBaseType, NodeTypQryMngrT extends UsrComnBaseQryMngr, TableT extends Table> extends MasterDetailScreen<NodeTypeT> {
    ...
    @Autowired
    protected CollectionLoader<UsrNodeBaseType> colLoadrMain;
    @Autowired
    protected CollectionContainer<UsrNodeBaseType> colCntnrMain;
    @Autowired
    protected InstanceContainer<UsrNodeBaseType> instCntnrMain;
    @Autowired
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<UsrNodeBaseType> thisNodeTypes = tableMain.getSelected().stream().toList();
    ...
    public void onUpdateColItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        List<UsrNodeBaseType> thisNodes = tableMain.getSelected().stream().toList();
    ...
    public void onInstCntnrMainItemChange(InstanceContainer.ItemChangeEvent<UsrNodeBaseType> event) {
        String logPrfx = "onInstCntnrMainItemChange";
        logger.trace(logPrfx + " --> ");

        UsrNodeBaseType thisNodeType = event.getSource().getItemOrNull();
        if (thisNodeType == null) {
    ...
    public void onUpdateInstItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        UsrNodeBaseType thisNodeType = instCntnrMain.getItemOrNull();
        if (thisNodeType == null) {
            logger.debug(logPrfx + " --- thisNodeType is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
