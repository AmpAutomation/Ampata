...
public abstract class UsrBaseNodeType0BaseMain<NodeTypeT extends UsrBaseNodeType, NodeTypQryMngrT extends UsrBaseQryMngr, TableT extends Table> extends MasterDetailScreen<NodeTypeT> {
    ...
    public UsrBaseNodeType0BaseMain() {
    ...
    //Query Manager
    protected NodeTypQryMngrT qryMngr;
    ...
    @Autowired
    protected CollectionLoader<UsrBaseNodeType> colLoadrMain;
    @Autowired
    protected CollectionContainer<UsrBaseNodeType> colCntnrMain;
    @Autowired
    protected InstanceContainer<UsrBaseNodeType> instCntnrMain;
    @Autowired
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<UsrBaseNodeType> thisNodeTypes = tableMain.getSelected().stream().toList();
    ...
    public void onUpdateColItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        List<UsrBaseNodeType> thisNodes = tableMain.getSelected().stream().toList();
    ...
    public void onInstCntnrMainItemChange(InstanceContainer.ItemChangeEvent<UsrBaseNodeType> event) {
        String logPrfx = "onInstCntnrMainItemChange";
        logger.trace(logPrfx + " --> ");

        UsrBaseNodeType thisNodeType = event.getSource().getItemOrNull();
        if (thisNodeType == null) {
    ...
    public void onUpdateInstItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        UsrBaseNodeType thisNodeType = instCntnrMain.getItemOrNull();
        if (thisNodeType == null) {
            logger.debug(logPrfx + " --- thisNodeType is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
