...
public abstract class SysNodeBaseMain<SysNodeT extends SysNode, SysNodeTypeT extends SysNodeType, SysNodeQryMngrT extends SysBaseQryMngr> extends MasterDetailScreen<SysNodeT> {
    ...
    //Query Manager
    protected SysNodeQryMngrT qryMngr;
    ...
    @Autowired
    protected EntityComboBox<SysNodeTypeT> tmplt_Type1_IdField;
    ...
    @Autowired
    protected InstanceContainer<SysNodeT> instCntnrMain;
    @Autowired
    protected TreeTable<SysNodeT> tableMain;

    
    //Type data container and loader
    protected CollectionContainer<SysNodeTypeT> colCntnrType;
    protected CollectionLoader<SysNodeTypeT> colLoadrType;
    ...
    @Autowired
    protected EntityPicker<SysNodeT> parent1_IdField;

    @Autowired
    protected EntityComboBox<SysNodeTypeT> type1_IdField;
    ...
    public void onInit(InitEvent event) {
        ...
        colCntnrType = dataComponents.createCollectionContainer(this.typeOfSysNodeTypeT);
        colLoadrType = dataComponents.createCollectionLoader();
        colLoadrType.setQuery("select e from enty_"+ this.typeOfSysNodeTypeT.getSimpleName() + " e order by e.id2");
        FetchPlan fchPlnType_Inst = fetchPlans.builder(this.typeOfSysNodeTypeT)
        ...
        //Filter
        EntityComboBox<SysNodeTypeT> propFilterCmpnt_Type1_Id;
        propFilterCmpnt_Type1_Id = (EntityComboBox<SysNodeTypeT>) filterConfig1A_Type1_Id.getValueComponent();
    ...
    public void onDuplicateBtnClick(Button.ClickEvent event) {
        ...
        List<SysNodeT> thisNodes = tableMain.getSelected().stream().toList();
        ...
        thisNodes.forEach(orig -> {
            SysNodeT copy = metadataTools.copy(orig);
            ...
            SysNodeT savedCopy = dataManager.save(copy);
    ...
    public void onInstCntnrMainItemChange(InstanceContainer.ItemChangeEvent<SysNodeT> event) {
        String logPrfx = "onInstCntnrMainItemChange[super]";
        logger.trace(logPrfx + " --> ");

        SysNodeT thisNode = event.getSource().getItemOrNull();
        if (thisNode == null) {
    ...
    public void onUpdateInstItemValsBtnClick(Button.ClickEvent event) {
        ...
        SysNodeT thisNode = instCntnrMain.getItemOrNull();
        if (thisNode == null) {
            logger.debug(logPrfx + " --- thisNode is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
    ...
    public void onType1_IdFieldValueChange(HasValue.ValueChangeEvent<SysNodeType> event) {
