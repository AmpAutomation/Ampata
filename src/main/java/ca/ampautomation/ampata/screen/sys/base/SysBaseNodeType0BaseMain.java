...
public abstract class SysBaseNodeType0BaseMain<NodeTypeT extends SysBaseNodeType, NodeTypeQryMngrT extends SysBaseQryMngr> extends MasterDetailScreen<NodeTypeT> {
    ...
    public SysBaseNodeType0BaseMain() {
    ...
    protected void updateHelper() {
        ...
        if(dataContext.hasChanges()) {
            ...
            thisNodeTypes.forEach(thisNodeType -> {
                //SysBaseNodeType thisTrackedNode = dataContext.merge(thisNodeType);
                if (thisNodeType != null) {
                    thisNodeType = dataContext.merge(thisNodeType);
    ...
    public void onInstCntnrMainItemChange(InstanceContainer.ItemChangeEvent<SysBaseNodeType> event) {
        String logPrfx = "onInstCntnrMainItemChange";
        logger.trace(logPrfx + " --> ");

        SysBaseNodeType thisNodeType = event.getSource().getItemOrNull();
        if (thisNodeType == null) {
    ...
    public void onUpdateInstItemCalcValsBtnClick(Button.ClickEvent event) {
        ...
        SysBaseNodeType thisNodeType = instCntnrMain.getItemOrNull();
        if (thisNodeType == null) {
            logger.debug(logPrfx + " --- thisNodeType is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
