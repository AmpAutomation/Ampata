...
import ca.ampautomation.ampata.screen.sys.base.SysBaseNode0BaseMain;
import io.jmix.core.*;
...
@UiDescriptor("sys-fin-curcy-exch-rate-0-main.xml")
@LookupComponent("tableMain")
public class SysFinCurcyExchRate0Main extends SysBaseNode0BaseMain<SysFinCurcyExchRate, SysFinCurcyExchRateType, SysFinCurcyExchRateQryMngr> {
    ...
    @Autowired
    protected PropertyFilter<SysFinCurcy> filterConfig1A_FinCurcy1_Id;

    @Autowired
    protected PropertyFilter<SysFinCurcy> filterConfig1A_FinCurcy2_Id;
    ...
    @Autowired
    protected EntityComboBox<SysFinCurcy> tmplt_FinCurcy1_IdField;
    @Autowired
    protected CheckBox tmplt_FinCurcy1_IdFieldChk;

    @Autowired
    protected EntityComboBox<SysFinCurcy> tmplt_FinCurcy2_IdField;
    @Autowired
    ...
    protected CollectionContainer<SysFinCurcy> colCntnrFinCurcy;
    protected CollectionLoader<SysFinCurcy> colLoadrFinCurcy;


    //Field
    @Autowired
    private EntityComboBox<SysFinCurcy> finCurcy1_IdField;

    @Autowired
    private EntityComboBox<SysFinCurcy> finCurcy2_IdField;
    ...
    public void onInit(InitEvent event) {
        ...
        //filter
        EntityComboBox<SysFinCurcy> propFilterCmpnt_FinCurcy1_Id;
        propFilterCmpnt_FinCurcy1_Id = (EntityComboBox<SysFinCurcy>) filterConfig1A_FinCurcy1_Id.getValueComponent();
        propFilterCmpnt_FinCurcy1_Id.setOptionsContainer(colCntnrFinCurcy);

        EntityComboBox<SysFinCurcy> propFilterCmpnt_FinCurcy2_Id;
        propFilterCmpnt_FinCurcy2_Id = (EntityComboBox<SysFinCurcy>) filterConfig1A_FinCurcy2_Id.getValueComponent();
    ...
    public void onDeriveBtnClick(Button.ClickEvent event) {
        ...
        List<SysFinCurcyExchRate> thisFinCurcyExchRate = tableMain.getSelected().stream().toList();
        ...
        List<SysFinCurcyExchRate> sels = new ArrayList<>();

        thisFinCurcyExchRate.forEach(orig -> {

            SysFinCurcyExchRate copy = metadataTools.copy(orig);
            ...
            SysFinCurcyExchRate savedCopy = dataManager.save(copy);
    ...
    public void onSetBtnClick(Button.ClickEvent event) {
        ...
        List<SysFinCurcyExchRate> thisFinCurcyExchRates = tableMain.getSelected().stream().toList();
    ...
    public void onFinCurcy1_IdFieldValueChange(HasValue.ValueChangeEvent<SysFinCurcyExchRate> event) {
        String logPrfx = "onFinCurcy1_IdFieldValueChange";
        logger.trace(logPrfx + " --> ");

        if (event.isUserOriginated()) {
            SysFinCurcyExchRate thisFinCurcyExchRate = instCntnrMain.getItemOrNull();
            if (thisFinCurcyExchRate == null) {
    ...
    public void onFinCurcy2_IdFieldValueChange(HasValue.ValueChangeEvent<SysFinCurcyExchRate> event) {
    ...
    public void onUpdateAmt2FieldBtnClick(Button.ClickEvent event) {
        ...
        SysBaseNode thisFinCurcyExchRate = instCntnrMain.getItemOrNull();
        if (thisFinCurcyExchRate == null) {
            logger.debug(logPrfx + " --- thisFinCurcyExchRate is null, likely because no record is selected.");
            notifications.create().withCaption("No record selected. Please select a record.").show();
            logger.trace(logPrfx + " <-- ");
