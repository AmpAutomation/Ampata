...
drop procedure if exists UsrNodeFinBal_Pr_Upd_AllCalcVals_ForAllRows;

create procedure UsrNodeFinBal_Pr_Upd_AllCalcVals_ForAllRows()
language 'plpgsql'
as $BODY$
declare
	rec_tenant record;
...
and t.deleted_by is null
and t2.dtype = 'enty_UsrNodeFinBalType'
;	
...
--ts2_el_ts
--fin_dept1__id
raise notice 'Updating fin_bal_set1__id2, ts1_el_ts, ts2_el_ts, fin_dept1__id';
update ampata_usr_node t
set 
	 fin_bal_set1__id2 = t1.id2
	,ts1_el_ts = t1.ts1_el_ts
	,ts2_el_ts = t1.ts2_el_ts
...
update ampata_usr_node t
set sys_node_fin_curcy1__id2 = t1.id2
from ampata_sys_node t1
where t.sys_node_fin_curcy1__id = t1.id
and	t.dtype = 'enty_UsrNodeFinBal'
and t.deleted_by is null
;	
...
loop
	...
	for rec_fin_bal in
		...
		and t.deleted_by is null
		and t.tenant = rec_tenant.tenant_id
		order by t.fin_acct1__id2, t.fin_dept1__id2, t.sys_node_fin_curcy1__id2, t.ts2_el_ts
		--limit 100
	loop
...
--		end if;
		...
		raise notice '--- i_tenant:% --- i_fin_bal:% rec_fin_bal.sys_node_fin_curcy1__id2:%', i_tenant, i_fin_bal, rec_fin_bal.sys_node_fin_curcy1__id2;
		raise notice '--- i_tenant:% --- i_fin_bal:% rec_fin_bal.ts1_el_ts:%', i_tenant, i_fin_bal, rec_fin_bal.ts1_el_ts;
		raise notice '--- i_tenant:% --- i_fin_bal:% rec_fin_bal.ts2_el_ts:%', i_tenant, i_fin_bal, rec_fin_bal.ts2_el_ts;
		...
		if rec_fin_bal.type1__id2 = '/Rng-Given-Year' then
			if rec_fin_bal.fin_dept1__id  is null then
				...
				and t.sys_node_fin_curcy1__id = rec_fin_bal.sys_node_fin_curcy1__id
				and t.ts1_el_ts = rec_fin_bal.ts1_el_ts - interval '1' year
				and t.ts2_el_ts = rec_fin_bal.ts2_el_ts - interval '1' year
...
--		elsif rec_fin_bal.type1__id2 == '/Rng-Given-Day' then
		...
		if rec_fin_bal.fin_dept1__id  is null then
			...
			and t.fin_acct1__id = rec_fin_bal.fin_acct1__id
			and t.sys_node_fin_curcy1__id = rec_fin_bal.sys_node_fin_curcy1__id
			and t.ts1_el_ts between rec_fin_bal.ts1_el_ts and rec_fin_bal.ts2_el_ts
...
with cte1 as(
	select
		row_number() over(
			partition by tenant, fin_acct1__id, sys_node_fin_curcy1__id
			order by id_dt_date1
			) as nr
		,id
		,id2
	...
	and deleted_by is null
--	and fin_acct1__id2 = '/Mark/A/RBC/Chk'
	order by tenant, fin_acct1__id, sys_node_fin_curcy1__id, id_dt_date1
)
...
from (
	...
	where t2a.dtype = 'enty_UsrNodeFinBal'
...
--sys_node_fin_curcy1__id, sys_node_fin_curcy1__id2
raise notice 'Updating sys_node_fin_curcy1__id, sys_node_fin_curcy1__id2';
update ampata_usr_node t
set  sys_node_fin_curcy1__id = t1.sys_node_fin_curcy1__id
	,sys_node_fin_curcy1__id2 = t1.sys_node_fin_curcy1__id2
from (
	select t2a.id
		,t2a.id2
		,t2a.fin_acct1__id
		,t2b.id2 as fin_acct1__id2
		,t2b.sys_node_fin_curcy1__id
		,t2c.id2 as sys_node_fin_curcy1__id2
	from ampata_usr_node t2a
	inner join ampata_usr_node t2b
	on t2a.fin_acct1__id = t2b.id
	inner join ampata_sys_node t2c
	on t2b.sys_node_fin_curcy1__id = t2c.id
	where t2a.dtype = 'enty_UsrNodeFinBal'
	and t2a.deleted_by is null
	and t2b.dtype = 'enty_UsrNodeFinAcct'
	and t2c.dtype = 'enty_UsrNodeFinCurcy'
) t1
where t.id = t1.id 
and	t.dtype = 'enty_UsrNodeFinBal'
and t.deleted_by is null
and t.sys_node_fin_curcy1__id is null
;
...
call UsrNodeFinBal_Pr_Upd_AllCalcVals_ForAllRows();
